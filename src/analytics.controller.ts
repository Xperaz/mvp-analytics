...
export class AnalyticsController {
  ...
  async createReport(@Body() body: any) {
    return this.analyticsService.createReport(
      body.name,
      body.querySql,
      body.createdBy
    );
  ...
  async generateReport(@Body() body: any) {
    ...
    const hasAccess = await this.analyticsService.validateUserAccess(
      body.userId,
      "reports"
    );
    if (!hasAccess) {
    ...
    const result: any = await this.analyticsService.generateReport(
      body.reportType,
      body.userId,
      body.dateRange
    );
  ...
  async processEvent(@Body() body: any) {
    const processedData = await this.analyticsService.ProcessEventData(
      body.rawData,
      body.userId
    );
    return this.analyticsService.trackEvent(
